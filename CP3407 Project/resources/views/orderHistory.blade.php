                        ...
                        @foreach ($orders as $order)
                            <tr>
                                ...
                                <td class="border px-4 py-2">
                                    <button onclick="toggleDetails({{ $order->id }})" class="text-blue-500">View Details</button>
                                </td>
                            </tr>
                                    ...
                                    <table class="table-auto w-full">
                                        ...
                                        <tbody>
                                            @foreach ($orderDetails as $detail)
                                                <tr class="border">
                                                    <td class=" px-4 py-2 flex justify-center items-center"><img class="w-20 h-20 " src="{{ $detail->product->p_photo }}" alt=""></td>
                                                    <td class=" px-4 py-2 text-left">{{ $detail->product->pname }}</td>
                                                    <td class=" px-4 py-2">{{ $detail->quantity }}</td>
                                                    <td class=" px-4 py-2">${{ number_format($detail->totalPrice) }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
